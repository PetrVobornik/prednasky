...
namespace Soubory
{
    public static class Soubor
    {
        ...
        public static void VytvorXml(string soubor)
        {
            XDocument doc = new XDocument(
                new XElement("data",
                    new XAttribute("name", "text"),
                    new XAttribute("datum", DateTime.Now),
                    new XElement("polozka", "Delší text uvnitř elementu",
                        new XAttribute("name", "počet"),
                        new XAttribute("count", 75)
                    ),
                    new XElement("polozka",
                        new XAttribute("name", "výška"),
                        new XAttribute("count", 57),
                        new XElement("polozka2")
                    )
                ));
            doc.Save(soubor);
        }

        public static void VytvorXml2(string soubor)
        {
            XDocument doc = new XDocument(new XElement("data"));

            doc.Root.SetAttributeValue("name", "text");
            doc.Root.SetAttributeValue("datum", DateTime.Now);

            XElement pol1 = new XElement("polozka");
            pol1.Value = "Delší text uvnitř elementu";
            pol1.SetAttributeValue("name", "počet");
            pol1.SetAttributeValue("count", 75);
            doc.Root.Add(pol1);

            XElement pol2 = new XElement("polozka");
            pol2.SetAttributeValue("name", "výška");
            pol2.SetAttributeValue("count", 57);
            pol2.Add(new XElement("polozka2"));
            doc.Root.Add(pol2);

            doc.Save(soubor);
        }

        public static void CtiXml(string soubor)
        {
            XDocument doc = XDocument.Load(soubor);

            Console.WriteLine("Název elementu = {0}", doc.Root.Name);
            Console.WriteLine("Name = {0}", doc.Root.Attribute("name").Value);
            Console.WriteLine("Datum = {0:dd.MM.yyyy HH:mm:ss}", (DateTime)doc.Root.Attribute("datum"));

            Console.WriteLine("Data z podelementů");
            foreach (var elm in doc.Root.Elements())
                Console.WriteLine("{0} = {1}", elm.Attribute("name").Value, elm.Attribute("count").Value);
            Console.WriteLine();

            Console.WriteLine("Všechny podelementy");
            foreach (var elm in doc.Root.Descendants())
            {
                Console.Write("{0}: ", elm.Name);
                if (elm.Attribute("name") != null)
                    Console.Write("{0} = {1}", elm.Attribute("name").Value, elm.Attribute("count")?.Value ?? "-");
                Console.WriteLine();
            }
        }
