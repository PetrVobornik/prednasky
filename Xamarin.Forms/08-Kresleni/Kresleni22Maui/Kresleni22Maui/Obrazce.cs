...
public abstract class Obrazec
{
    void ChangeRec(double? x = null, double? y = null, double? width = null, double? height = null)
    {
        ...
        AbsoluteLayout.SetLayoutBounds(Prvek,
            new Rect(x ?? rec.X, y ?? rec.Y, width ?? rec.Width, height ?? rec.Height));
    }
    ...
    public Obrazec()
    {
        ...
        AbsoluteLayout.SetLayoutBounds(Prvek, new Rect());
        Zleva = 10;
        Shora = 10;
        Sirka = 120;
        Vyska = 90;
        BarvaCary = Colors.Blue;
        BarvaVyplne = Colors.Yellow;
        TloustkaCary = 3;
    }
    ...
    public virtual void NactiZXml(XElement elm)
    {
        GetType().GetProperties()
            ...
            .ForEach(p => {
                ...
                else if (p.PropertyType == typeof(Color)) p.SetValue(this, Color.FromArgb(elm.Attribute(p.Name).Value));
            });
    }
}
